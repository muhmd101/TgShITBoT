...
async def mute_user(user: client.Client, msg: Message):
    ...
    if msg.reply_to_message:
        if msg.reply_to_message.sender_chat:
            target = msg.reply_to_message.sender_chat
            is_channel = True
        elif msg.reply_to_message.from_user:
            target = msg.reply_to_message.from_user
    elif len(msg.command) > 1:
        arg = msg.command[1]
        arg_val = int(arg) if arg.lstrip("-").isdigit() else arg
        try:
            target = await user.get_users(arg_val)
        except Exception:
            try:
                target = await user.get_chat(arg_val)
                is_channel = True
            except Exception:
                return await msg.edit_text(
                    f"{get_emoji('CrossMark', markdown=True)} **Couldn't find that user/channel.**"
                )
    
    if not target:
        return await msg.edit_text(
            f"{get_emoji('CrossMark', markdown=True)} **Reply to a user/channel or provide an ID/username.**"
        )

    await user.db.add_muted_global(target.id)
    mention = f"**{target.title}**" if is_channel else target.mention
    
    await msg.edit_text(
        f"{get_emoji('CheckMark', markdown=True)} **Muted** {mention} **globally.**"
    )
...
async def unmute_user(user: client.Client, msg: Message):
    ...
    await msg.edit_text(
        f"{get_emoji('CheckMark', markdown=True)} **Unmuted** {mention} **globally.**"
    )
...
async def muted_list(user: client.Client, msg: Message):
    ...
    if not muted_ids:
        return await msg.edit_text(
            f"{get_emoji('who', markdown=True)} **No globally muted users/channels.**"
        )
    lines = []
    for uid in muted_ids:
        try:
            ...
            try:
                chat = await user.get_chat(uid)
                lines.append(f"• **{chat.title}** (`{uid}`)")
            except Exception:
                lines.append(f"• `{uid}` (unresolved)")
    text = (
        f"{get_emoji('who', markdown=True)} **Globally muted users/channels ({len(muted_ids)}):**\n"
        + "\n".join(lines)
    )
